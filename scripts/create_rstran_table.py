...
EXCEL_PATH = Path(
    os.getenv(
        "RSTRAN_TABLE_DEFINITION_PATH",
        str(BASE_DIR / "Table-Template" / "table_definition_rstran.xlsx"),
    )
)
TEMPLATE_REQUIRED = os.getenv("RSTRAN_TEMPLATE_REQUIRED", "false").strip().lower() == "true"
...
def resolve_template_path() -> Path | None:
    if EXCEL_PATH.exists():
        return EXCEL_PATH
    alt = BASE_DIR / "Table Definition" / "table_definition_rstran.xlsx"
    if alt.exists():
        return alt
    return None
...
def main():
    ...
    if resolved_path is None:
        if TEMPLATE_REQUIRED:
            raise FileNotFoundError(f"RSTRAN table definition not found: {EXCEL_PATH}")
        print(f"[create_rstran_table] Table definition file not found, skipping: {EXCEL_PATH}")
        return
